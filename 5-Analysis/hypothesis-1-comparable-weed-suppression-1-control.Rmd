...
fsb.weeds.N <- fsb %>% filter(!treatment %in% c("P", "O")) #retain the first control treatment only

weeds.lm1 <- lm(log(weed.biomass.g.per.sq.m + 1) ~ as.factor(block) + treatment + crop.biomass.g.per.sq.m, data = fsb.weeds.N)


weeds.lm2 <- lm(log(weed.biomass.g.per.sq.m + 1) ~ as.factor(block) + treatment , data = fsb.weeds.N)
