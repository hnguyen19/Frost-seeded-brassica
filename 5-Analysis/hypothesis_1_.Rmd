...
Using `ggResidpanel` version 0.3.0 (Goode and Rey, 2019) for model diagnosis, no predictable pattern in the plots of residuals vs. predicted values suggests that the analysis models fit the data well (Figures \@ref(fig:diag-crops) and \@ref(fig:diag-weeds)).   
...
```{r diag-crops, fig.cap = "Diagnosis plot for the linear model of crop biomass responding to treatment and weed biomass"}
...
crops.ancova <- joint_tests(crops.lm) %>% print(export = TRUE)
weeds.ancova <- joint_tests(weeds.lm) %>% print(export = TRUE)
```

```{r, eval=FALSE, include=FALSE}
cbind( weeds.ancova$summary, crops.anova$summary) %>%
  kable(caption = "ANOVA tables of crop and weed responses") %>%
  add_header_above(c(" " = 1,  "Weed" = 5, "Crop" = 5))
```

```{r ancova-weeds, echo=FALSE}
weeds.ancova.df <- as.data.frame(weeds.anova$summary)
...
```{r, echo=FALSE}
weeds.emm <- emmeans(weeds.lm, "treatment", type = "response")
...
weeds.emmip <- emmip(weeds.lm, ~ treatment, type = "response", plotit = FALSE)
weeds.emmip %>% 
  mutate(sd = 2*SE) %>%
...
crops.emmip <- emmip(crops.lm, ~ treatment , type = "response", plotit = FALSE)
crops.emmip %>% 
  mutate(sd = 2*SE) %>%
  ggplot(aes(x = treatment, y = yvar)) +
  geom_bar(stat="identity") + 
